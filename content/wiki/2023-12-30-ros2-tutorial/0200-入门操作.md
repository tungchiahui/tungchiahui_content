...
2. 创建 Dolphin 服务菜单：
   ...
   `%f` 会由 Dolphin 替换为当前选中的文件夹路径，启动脚本会先进入该目录，再打开 Warp。

3. 设置权限、刷新 KDE 缓存并重启 Dolphin：
...
重新打开 Dolphin 后，右键文件夹即可看到“在此处打开 Warp”。Dolphin 的 F4 内嵌终端依赖 KonsolePart，仍然只能使用 Konsole，与这个右键菜单无关。
