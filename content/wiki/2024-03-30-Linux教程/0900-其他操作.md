...
然后还有ipv6也要进行放行，那个debian一般默认ipv6不设防，但是会被上游openwrt防住，所以，要在openwrt里开启对应规则。
...
![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1788528028821-5f4711e7.webp)

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1788528057933-67c6e554.webp)
...
![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787154451682-f5d538c1.webp)

去`/opt/1panel/apps/alist/alist/data/data`目录修改掉如下图的设置从`false`改成`true`。
...
![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787154352253-3de75e7f.webp)

最后点击重启。

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787154532487-c9f49f91.webp)
...
如果显示`HTTP/1.1 200 OK`，那证明是OK的。

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787154636455-dff7a529.webp)

后续我也把他反向代理成了一个域名`s3.tungchiahui.cn`，经测试，也没毛病：

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787155243623-c19dfc25.webp)

##### 配置S3

在AList的管理界面找到设置里的S3,然后点底下的Add来添加哪个路径为桶：
- Name就是桶的名字
- Path是你想让AList的哪个路径作为桶子的根目录
- S3 access key id 是S3授权ID
- S3 secret access key是S3授权密钥

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787154999413-e747a0fd.webp)
...
![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787156017266-fa0d943f.webp)

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787156156306-94170611.webp)
...
如果开着 Sign All，关掉。因为只要全局 Sign All 开着，下面单独存储关签名也没用。AList 官方明确说 Sign All 会给所有文件直链添加签名。

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787156596929-05141566.webp)

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787156612532-01b09796.webp)

然后找到你的存放AList的桶子的存储地点进行编辑。

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787156652655-1141dad8.webp)

关闭`启用签名 / Enable signing`。

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787156722613-8c9e13a7.webp)

然后保存。

然后你可以通过`![](https://alist.tungchiahui.cn/d/AList文件的路径/文件名.webp)`来访问文件了。

**高阶操作（可选）**：

把`cdn.tungchiahui.cn`也和`alist.tungchiahui.cn`同源，然后在腾讯云edgeone里加速，在规则引擎里新加一个`cdn路径前缀增加`的规则。

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787159371048-e2e3749f.webp)

- HOST选择`cdn.tungchiahui.cn`
- 操作选`回源URL重写`
- 类型选`增加路径前缀`
- 增加路径前缀选`/d/AList文件的多余路径/`
  
比如`增加路径前缀`我填的`/d/NAS/ObjectStorage`，
那么我用`https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787159371048-e2e3749f.webp`
将会同等于`https://alist.tungchiahui.cn/d/NAS/ObjectStorage/tungwebsite/assets/images/2024/03/30/1787159371048-e2e3749f.webp`,
都会回源到我的`https://服务器ip地址/d/NAS/ObjectStorage/tungwebsite/assets/images/2024/03/30/1787159371048-e2e3749f.webp`。


再设置一个缓存规则：

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787160214979-6bf621c3.webp)

缓存规则可以这样理解：

- **节点缓存 TTL：30 天**  
  EdgeOne 会把图片缓存到边缘节点 30 天。后续访问优先从 CDN 节点返回，可以明显减少 AList、服务器 和 服务器宽带 的压力。

- **强制缓存：关闭**  
  保持关闭更稳妥。如果源站明确要求某些资源不要缓存，EdgeOne 会尊重源站设置，避免把特殊资源错误缓存。
...
- **Cache Key：查询字符串全部忽略**  
  ...
  都会被认为是同一个缓存资源。这样可以防止别人不断修改 URL 参数绕过缓存，反复回源打你的 AList 和 服务器，而我这里文件还存储在NAS上，也会进一步防止他反复打我的NAS。

- **忽略大小写：关闭**  
  `abc.webp` 和 `ABC.webp` 会被当成两个不同文件，避免不同文件被错误合并到同一个缓存。

- **PHP / JSP / ASP / ASPX：不缓存**  
  这些通常属于动态页面，单独设为不缓存可以防止动态内容被 CDN 长时间缓存。对于纯图片 CDN 来说基本属于防呆设置。

- **图片浏览器缓存 TTL：1 天**  
  `jpg`、`png`、`gif`、`bmp`、`svg`、`webp` 等图片会在用户浏览器中缓存 1 天。用户短时间内重复访问时，浏览器可以直接读取本地缓存，连 EdgeOne 都不用重新请求。
...
绝大多数重复访问会停在浏览器或 EdgeOne，不会真正回源到家里的 服务器 和 NAS。
...
| 数据冗余        | 腾讯负责          | **你负责**                |
| 硬盘损坏        | 云厂商处理         | **可能直接丢数据**            |
| 运维          | 几乎没有          | Docker、磁盘、备份、升级、安全都自己搞 |
| 费用          | 存储+流量         | 软件可以自建，但硬盘/服务器/带宽自己承担  |
| CDN         | 很容易接腾讯 CDN/EO | MinIO 本身**不是 CDN**     |
| 可控性         | 一般            | **非常高**                |


#### 安装

首先你需要服务器上安装一个`1panel`.

进入商店,搜索minio.

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787145834399-2d3916b0.webp)

点击安装.

大多数设置都默认即可,但账号密码要自己填写,这个端口外部访问这里,如果你的服务器在内网,可以选择打开,如果你的服务器的IP是一个公网IP,那么不建议你打开.

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787145965960-68b6e978.webp)

| 配置项       | 建议                 |
| --------- | ------------------ |
| 名称        | `minio`            |
| 版本        | 选 1Panel 当前提供的稳定版本 |
| 用户        | 自己设一个，不要用太简单的      |
| 密码        | 自己设一个密码复杂点的              |
| WebUI 端口  | `9001`             |
| S3 API 端口 | `9000`             |
...
| 启用 WebUI  | 开                  |
| WebUI 动画  | 随意                 |
| 端口外部访问    | **建议不开公网暴露,但如果测试的话,先开公网暴露也可以.**      |

最后点击安装.

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787146064891-460b603b.webp)
...
比如我这个内网`10.0.0.4:9001`

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787147091527-f99d5032.webp)
...
![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787147218585-a9d82f0b.webp)

再随便上传一张图片试试.

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787147308943-431e0c9c.webp)

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787147318544-4cd5e37f.webp)

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787147349844-1421edc4.webp)

##### 测试S3协议

S3协议的网址应该是`服务器ip:9000`:

比如我这个内网`10.0.0.4:9000`
...
![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787147506873-538da12a.webp)
...
![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787147577638-0f060165.webp)


##### 接入AList测试

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787148971335-5ead1508.webp)

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787149002738-17bac9d7.webp)

这个一定要开着。

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787149013096-2f2eba16.webp)

配置完后，可以返回文件列表，发现刚才的文件可以被正常查看了。

![](https://cdn.tungchiahui.cn/tungwebsite/assets/images/2024/03/30/1787149035102-0dba3465.webp)
