...
```bash
g++ demo1_serial_write.cpp -o demo1_serial_write -std=c++23 -lboost_system -pthread
...
int main(int argc, char* argv[])
{
    ...
    std::println("main：等待串口收到一行数据，以换行符结束");
    ...
    std::println("main：读取到字节数 = {}", n);
    std::println("main：读取到一行 = {}", line);
...
```bash
g++ demo2_serial_read_line.cpp -o demo2_serial_read_line -std=c++23 -lboost_system -pthread
                  ...
                  boost::asio::streambuf* buffer)
{
    if (ec)
    {
        std::println("on_read_line：读取失败：{}", ec.message());
        return;
    }
    ...
    std::println("on_read_line：bytes_transferred = {}", bytes_transferred);
    std::println("on_read_line：收到一行 = {}", line);
}
...
int main(int argc, char* argv[])
{
    ...
    std::println("main：注册 async_read_until");
    ...
    std::println("main：async_read_until 已返回，准备 io.run()");
...
```bash
g++ demo3_async_serial_read.cpp -o demo3_async_serial_read -std=c++23 -lboost_system -pthread
...
public:
        ...
        : serial_(io)
    {
        ...
        std::println("SerialLineReader：串口打开成功，开始异步读取");
...
private:
    ...
    void on_read(const boost::system::error_code& ec, std::size_t bytes_transferred)
    {
        if (ec)
        {
            std::println("SerialLineReader::on_read 错误：{}", ec.message());
            return;
        }
        ...
        std::println("收到串口行，bytes = {}，内容 = {}", bytes_transferred, line);
...
int main(int argc, char* argv[])
{
    ...
    std::println("main：调用 io.run()，按 Ctrl+C 退出");
...
```bash
g++ demo4_serial_class_reader.cpp -o demo4_serial_class_reader -std=c++23 -lboost_system -pthread
...
private:
    ...
    void on_write(const boost::system::error_code& ec, std::size_t bytes_transferred)
    {
        ...
        std::println("发送完成，字节数 = {}", bytes_transferred);
