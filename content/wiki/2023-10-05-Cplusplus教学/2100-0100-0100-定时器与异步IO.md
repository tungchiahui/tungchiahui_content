...
```bash
g++ demo3_two_timers.cpp -o demo3_two_timers -std=c++23 -lboost_system -pthread
./demo3_two_timers
```
...
```bash
g++ demo4_repeat_timer.cpp -o demo4_repeat_timer -std=c++23 -lboost_system -pthread
./demo4_repeat_timer
```
...
```bash
g++ demo5_class_timer.cpp -o demo5_class_timer -std=c++23 -lboost_system -pthread
./demo5_class_timer
```
...
```bash
g++ demo6_class_two_timers.cpp -o demo6_class_two_timers -std=c++23 -lboost_system -pthread
...
```bash
g++ demo7_two_threads.cpp -o demo7_two_threads -std=c++23 -lboost_system -pthread
./demo7_two_threads
```
...
private:
    void on_timer(const boost::system::error_code& ec, int id)
    {
        if (ec)
        {
            std::println("timer{} 取消：{}", id, ec.message());
            return;
        }

        ++count_;

        std::println("timer{} 回调执行，count = {}，线程 id = {}", id, count_, std::this_thread::get_id());
    }
...
{
    std::println("{} 开始 run，线程 id = {}", name, std::this_thread::get_id());
...
```bash
g++ demo8_strand.cpp -o demo8_strand -std=c++23 -lboost_system -pthread
./demo8_strand
```
