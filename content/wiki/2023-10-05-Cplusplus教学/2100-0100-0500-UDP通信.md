...
int main()
{
    ...
    std::println("receiver：监听 UDP 0.0.0.0:9001，等待数据");

    // receive_from 会等待并接收一个 UDP 数据报。
    std::size_t n = socket.receive_from(boost::asio::buffer(data), sender_endpoint, 0, ec);
    if (ec)
    {
        std::println("receiver：接收失败：{}", ec.message());
        return 1;
    }

    std::string msg(data.data(), n);

    std::println("receiver：收到来自 {}:{} 的 {} 字节：{}", sender_endpoint.address().to_string(), sender_endpoint.port(), n, msg);
...
```bash
g++ demo1_udp_receiver.cpp -o demo1_udp_receiver -std=c++23 -lboost_system -pthread
./demo1_udp_receiver
```
...
int main()
{
    ...
    if (ec)
    {
        std::println("sender：打开 socket 失败：{}", ec.message());
        return 1;
    }
    ...
    std::println("sender：准备发送到 127.0.0.1:9001");

    // send_to 指定目标地址发送 UDP 数据报。
    std::size_t n = socket.send_to(boost::asio::buffer(msg), receiver_endpoint, 0, ec);
    if (ec)
    {
        std::println("sender：发送失败：{}", ec.message());
        return 1;
    }

    std::println("sender：发送完成，字节数 = {}", n);
...
```bash
g++ demo2_udp_sender.cpp -o demo2_udp_sender -std=c++23 -lboost_system -pthread
./demo2_udp_sender
```
                ...
                udp::endpoint* sender_endpoint)
{
    if (ec)
    {
        std::println("on_receive：接收失败：{}", ec.message());
        return;
    }

    std::string msg(data->data(), bytes_transferred);

    std::println("on_receive：收到来自 {}:{} 的 {} 字节：{}", sender_endpoint->address().to_string(), sender_endpoint->port(), bytes_transferred, msg);
}
...
int main()
{
    ...
    std::println("main：注册 async_receive_from");
    ...
    std::println("main：async_receive_from 已返回，准备 io.run()");
...
```bash
g++ demo3_async_udp_receiver.cpp -o demo3_async_udp_receiver -std=c++23 -lboost_system -pthread
...
public:
    ...
    {
        std::println("UdpEchoServer：监听 UDP 端口 {}", port);
        start_receive();
    }
...
private:
    ...
    void on_receive(const boost::system::error_code& ec, std::size_t bytes_transferred)
    {
        if (ec)
        {
            std::println("UdpEchoServer：接收失败：{}", ec.message());
            start_receive();
            return;
        }
        ...
        std::println("收到 {}:{} 的数据：{}", remote_endpoint_.address().to_string(), remote_endpoint_.port(), msg);
    ...
    void on_send(const boost::system::error_code& ec, std::size_t bytes_transferred)
    {
        if (ec)
        {
            std::println("UdpEchoServer：发送失败：{}", ec.message());
        }
        else
        {
            std::println("回显完成，字节数 = {}", bytes_transferred);
        }
...
```bash
g++ demo4_udp_echo_server.cpp -o demo4_udp_echo_server -std=c++23 -lboost_system -pthread
...
private:
    ...
    void on_timer(const boost::system::error_code& ec)
    {
        if (ec)
        {
            std::println("timer 取消：{}", ec.message());
            return;
        }
    ...
    void on_send(const boost::system::error_code& ec, std::size_t bytes_transferred)
    {
        ...
        std::println("发送 {}，字节数 = {}", msg_, bytes_transferred);
...
```bash
g++ demo5_udp_heartbeat.cpp -o demo5_udp_heartbeat -std=c++23 -lboost_system -pthread
./demo5_udp_heartbeat
```
