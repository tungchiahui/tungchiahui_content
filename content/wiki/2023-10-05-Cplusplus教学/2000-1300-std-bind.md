...
### 示例 6：在成员函数中使用 `this` 绑定当前对象

前面的示例是在类外部创建对象，再把对象地址 `&printer` 传给 `std::bind`。如果 `std::bind` 本身就在类的成员函数中使用，可以直接传入 `this`，表示把成员函数绑定到当前对象。
...
| 参数 | 含义 |
|:---|:---|
| `&Robot::on_message` | 要调用的成员函数 |
| `this` | 调用这个成员函数的当前对象 |
| `_1` | `callback()` 调用时传入的第一个参数 |
| `_2` | `callback()` 调用时传入的第二个参数 |

非静态成员函数不能脱离对象单独调用。`&Robot::on_message` 只说明了“要调用哪个成员函数”，还必须告诉 `std::bind`“在哪个 `Robot` 对象上调用”。
...
而在 `Robot` 自己的成员函数内部，`this` 就是当前对象的指针，所以可以直接写：
...
进入 `robot.demo()` 后，`demo()` 内部的 `this` 就指向 `robot`。
...
> **告诉 `std::bind`：以后调用这个成员函数时，就在当前这个对象上调用。**

这也是 ROS2 和 Boost.Asio 中很常见的写法，例如：

```cpp
std::bind(&MyNode::callback, this, _1)
```

它表示把 `MyNode::callback` 绑定到当前这个 `MyNode` 对象上，再把调用时的第一个参数传给 `callback`。

需要注意，`this` 本质上是一个指针。绑定后的回调如果活得比当前对象更久，对象销毁后再调用回调，就可能访问已经失效的 `this`，因此异步回调中仍然要注意对象生命周期。
...
| `this` | 成员函数内部的当前对象指针 | 常作为绑定成员函数时的第二个参数 |
...
- 成员函数中可以把 `this` 作为第二个参数，表示把成员函数绑定到当前对象。
