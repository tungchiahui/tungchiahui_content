...
在学习范围 for 之前，先回顾一下传统 `for` 如何遍历 STL 容器。假设有一个 `vector`：
...
下标写法直观，但只适用于 `vector`、`array` 等支持下标访问的容器；迭代器写法可以用于更多 STL 容器，但代码较长。范围 for 隐藏了 `begin()`、`end()` 和迭代器递增等细节，更直接地表达“依次处理容器中的每个元素”。
