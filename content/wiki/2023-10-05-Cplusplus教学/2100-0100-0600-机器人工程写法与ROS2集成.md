...
void parse_line(const std::string& line)
{
    ...
    if (type == "ODOM")
    {
        ...
        std::println("解析里程计：x = {}，y = {}，yaw = {}", x, y, yaw);
    }
    else if (type == "BAT")
    {
        double voltage = 0.0;
    ...
    else if (type == "IMU")
    {
        ...
        std::println("解析 IMU 加速度：{}, {}, {}", ax, ay, az);
    }
    else
    {
        std::println("未知协议行：{}", line);
    }
}
...
```bash
g++ demo1_protocol_parse.cpp -o demo1_protocol_parse -std=c++23
...
private:
    void parse_odom(std::istringstream& iss)
    {
        ...
        std::println("RobotProtocolParser：ODOM x = {}，y = {}，yaw = {}", x, y, yaw);
    }
    ...
    void parse_battery(std::istringstream& iss)
    {
        ...
        std::println("RobotProtocolParser：BAT voltage = {} V", voltage);
    }
};
...
```bash
g++ demo2_protocol_parser_class.cpp -o demo2_protocol_parser_class -std=c++23
...
public:
    void parse_line(const std::string& line)
    {
        ...
        if (type == "ODOM")
        {
            ...
            std::println("解析 ODOM：x = {}，y = {}，yaw = {}", x, y, yaw);
        }
        else if (type == "BAT")
        {
            double voltage = 0.0;
            ...
            std::println("解析 BAT：{} V", voltage);
        }
        else
        {
            std::println("未知数据：{}", line);
        }
    }
};
...
public:
        ...
        : serial_(io)
    {
        ...
        std::println("SerialRobotDriver：串口打开成功");
...
private:
    ...
    void on_read(const boost::system::error_code& ec, std::size_t bytes_transferred)
    {
        ...
        std::println("收到串口行，bytes = {}，line = {}", bytes_transferred, line);
...
```bash
g++ demo3_serial_robot_driver.cpp -o demo3_serial_robot_driver -std=c++23 -lboost_system -pthread
...
private:
    void run_asio()
    {
        std::println("Asio 线程：io.run() 开始");
        // 启动事件循环，前面注册的异步任务会在这里被调度执行。
        io_.run();
        std::println("Asio 线程：io.run() 返回");
    }
...
{
    std::print("Asio 线程中发送命令：{}", command);
