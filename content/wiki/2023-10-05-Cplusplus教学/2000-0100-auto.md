...
### 示例 5：在示例 4 基础上，auto 返回值和 decltype(auto) 的区别(指针)
...
### 示例 6：在示例 5 基础上，auto 返回值和 decltype(auto) 的区别(引用)
...
| 问题 | 示例 5（引用） | 示例 6（指针） |
|---|---|---|
| `score_ref()` / `score_ptr()` 返回什么 | `int&`（绑定到 `global_score` 本身） | `int*`（一个保存 `global_score` 地址的值） |
| `auto a = score_ref();` 拷贝的是什么 | `int` 的值 → 和 `global_score` 脱钩 | —— |
| `auto p1 = score_ptr();` 拷贝的是什么 | —— | "指针变量"这个值（地址）→ 但地址指向的还是 `global_score` |
| 修改 `a` / `*p1` 能否影响 `global_score` | `a = 90` 不能 | `*p1 = 90` 能 |
| `auto` 经过函数包一层（`read_score`/`read_ptr`）后还能改到原变量吗 | 不能（引用性已丢失） | 能（指针的指向关系从一开始就不依赖"引用性"） |
| `decltype(auto)` 相比 `auto` 的意义 | 关键：保住了 `int&`，否则会退化成 `int` 拷贝 | 没有额外意义：函数返回的本来就是 `int*`（一个普通值），`decltype(auto)` 和 `auto` 推导结果完全一致 |
 

`auto` 真正"丢掉"的东西只有一种：**表达式的引用性（即"是否和某个已有对象绑定"这件事）**。
 
- 对 `score_ref()`（返回 `int&`）而言，引用性就是它的全部价值所在——一旦丢了，`a` 就成了和原变量毫无关系的副本。`decltype(auto)` 的作用就是把这层引用性保住。
- 对 `score_ptr()`（返回 `int*`）而言，它本身就不是引用，是一个**普通的值**（哪怕这个值碰巧是个地址）。`auto` 推导普通值类型时本来就不会发生"丢引用"的事，所以推导出来天然就是 `int*`，和 `decltype(auto)` 推导结果没有任何区别。
**真正决定"能不能改到原变量"的，从来不是 `auto` 还是 `decltype(auto)`，而是你拷贝的这个值里，是否仍然携带着指向原对象的"路径"**：
 
- 拷贝 `int` 值 —— 路径断了，改不到。
- 拷贝 `int*`（地址值） —— 路径还在，*解引用*之后照样能改到。
