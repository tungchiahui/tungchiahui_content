...
int main()
{
    ...
    std::println("server：监听 0.0.0.0:9000，等待客户端连接");
    ...
    if (ec)
    {
        std::println("server：accept 失败：{}", ec.message());
        return 1;
    }

    std::println("server：客户端已连接");

    std::array<char, 1024> data;
    std::size_t n = socket.read_some(boost::asio::buffer(data), ec);
    if (ec)
    {
        std::println("server：读取失败：{}", ec.message());
        return 1;
    }

    std::string msg(data.data(), n);
    std::print("server：收到 {} 字节：{}", n, msg);

    boost::asio::write(socket, boost::asio::buffer(data, n), ec);
    if (ec)
    {
        std::println("server：回写失败：{}", ec.message());
        return 1;
    }

    std::println("server：已原样回写，程序结束");
...
```bash
g++ demo1_tcp_sync_server.cpp -o demo1_tcp_sync_server -std=c++23 -lboost_system -pthread
...
int main()
{
    ...
    std::println("client：准备连接 127.0.0.1:9000");

    // 发起连接，成功后 socket 才能收发数据。
    socket.connect(server_endpoint, ec);
    if (ec)
    {
        std::println("client：连接失败：{}", ec.message());
        return 1;
    }

    std::println("client：连接成功");

    std::string msg = "hello tcp\n";
    boost::asio::write(socket, boost::asio::buffer(msg), ec);
    if (ec)
    {
        std::println("client：发送失败：{}", ec.message());
        return 1;
    }

    std::println("client：发送完成，等待回显");
    ...
    if (ec)
    {
        std::println("client：读取失败：{}", ec.message());
        return 1;
    }

    std::string reply(data.data(), n);
    std::print("client：收到回显：{}", reply);
...
```bash
g++ demo2_tcp_sync_client.cpp -o demo2_tcp_sync_client -std=c++23 -lboost_system -pthread
...
private:
    ...
    void on_read(const boost::system::error_code& ec, std::size_t length)
    {
        if (ec)
        {
            std::println("session：读取结束或失败：{}", ec.message());
            return;
        }

        std::println("session：收到 {} 字节，准备回写", length);
    ...
    void on_write(const boost::system::error_code& ec, std::size_t length)
    {
        if (ec)
        {
            std::println("session：写失败：{}", ec.message());
            return;
        }

        std::println("session：回写完成，字节数 = {}", length);
...
int main()
{
    ...
    std::println("main：调用 io.run()，服务端开始运行");
...
```bash
g++ demo3_async_tcp_echo_server.cpp -o demo3_async_tcp_echo_server -std=c++23 -lboost_system -pthread
...
private:
    void on_connect(const boost::system::error_code& ec)
    {
        ...
        std::println("client：连接成功，开始异步发送");
    ...
    void on_write(const boost::system::error_code& ec, std::size_t length)
    {
        ...
        std::println("client：发送完成，字节数 = {}，等待回显", length);
...
```bash
g++ demo4_async_tcp_client.cpp -o demo4_async_tcp_client -std=c++23 -lboost_system -pthread
