...
```bash
g++ demo1_empty_run.cpp -o demo1_empty_run -std=c++23 -lboost_system -pthread
./demo1_empty_run
```
...
```bash
g++ demo2_post.cpp -o demo2_post -std=c++23 -lboost_system -pthread
./demo2_post
```
                   ...
                   std::shared_ptr<WorkGuard> guard)
{
    if (ec)
    {
        std::println("release_guard 定时器取消：{}", ec.message());
        return;
    }

    std::println("2 秒到了：释放 work_guard");
    guard->reset();
}
...
int main()
{
    ...
    std::println("main：有 work_guard，io.run() 不会空转退出");
...
```bash
g++ demo3_work_guard.cpp -o demo3_work_guard -std=c++23 -lboost_system -pthread
./demo3_work_guard
```
...
```bash
g++ demo4_bind_basic.cpp -o demo4_bind_basic -std=c++23
./demo4_bind_basic
```
...
```bash
g++ demo5_bind_member.cpp -o demo5_bind_member -std=c++23
./demo5_bind_member
```
...
```bash
g++ demo6_buffer.cpp -o demo6_buffer -std=c++23 -lboost_system -pthread
./demo6_buffer
```
...
```bash
g++ demo7_iothread.cpp -o demo7_iothread -std=c++23 -lboost_system -pthread
./demo7_iothread
```
