...
这里的`std::stoi`函数，他的转换规则是：从字符串开头（必须数字为开头）开始，尽可能解析一个整数；只要能解析出一部分，就算 stoi 本身成功。
例如：
- 转换`"123"`字符串，结果是成功的，为`123`。
- 转换`"123abc"`字符串，结果是成功的，为`123`。
- 转换`"3.14"`字符串，结果是成功的，为`3`。

而这个函数的`pos`参数表示`std::stoi`解析到了字符串的哪个位置，也就是第一个没有被转换的字符的位置。
例如：
- 转换`"123abc"`字符串，`val`是123,但`pos`是3。（序号从0开始，如下图）
...
- 转换`"3.14"`字符串，`val`是3,但`pos`是1。（序号从0开始，如下图）
...
这里的`pos == s.size()`是不成立的，所以返回`std::nullopt`。
